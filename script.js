...
function playRandomMelody() {
    const notes = ['q', 'e', 't', 'u', 'i', 'o', 'z', 'x', 'c', 'v', 'b', 'm'];
    let r_note = notes[Math.floor(Math.random() * notes.length)];

    const rhythmPattern = [0.8, 0.4, 0.4, 0.8, 0.4, 0.4, 0.8, 0.8];
    ...
    function playPhrase() {
        ...
        for (let i = 0; i < 4; i++) {
            for (const duration of rhythmPattern) {
                const dis = [-3, -2, -1, 0, 1, 2, 3][Math.floor(Math.random() * 7)];
                r_note = notes[(notes.indexOf(r_note) + dis + notes.length) % notes.length];
                nigun.push(r_note);
                shlus.push(duration);
            }
        }
        ...
        function playNextNote() {
            if (index < nigun.length) {
                playTune(nigun[index]);
                setTimeout(playNextNote, shlus[index] * 1000);
                index++;
            } else {
...
playRandomBtn.addEventListener('click', playRandomMelody);
